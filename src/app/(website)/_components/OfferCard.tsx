...
export default function SimpleOfferGallery() {
  ...
  const { data: offerData, isLoading, isError, error } = useQuery({
    queryKey: ["offerData"],
    queryFn: async () => {
    ...
    <section className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/30 py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {offerData?.data?.map((offer: Offer, index: number) =>
            offer.products.map((product) => (
              ...
              >
                ...
                <div className="relative overflow-hidden">
                  ...
                  <div
                    className="absolute top-12 left-8 w-2 h-2 bg-orange-300 rounded-full opacity-40 animate-pulse"
                    style={{ animationDelay: "1s" }}
                  ></div>
                  <div
                    className="absolute top-8 left-12 w-1 h-1 bg-yellow-300 rounded-full opacity-60 animate-ping"
        ...
        <div
          className="fixed bottom-20 left-10 w-32 h-32 bg-gradient-to-r from-purple-400/20 to-pink-400/20 rounded-full blur-2xl animate-bounce hidden lg:block"
